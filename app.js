...
function indelingCollapseStandaardToepassen() {
  ...
  // "Bij-/aanbouwen en buitenvoorzieningen" ook standaard ingeklapt (20-09-2026, tweede ronde) —
  // zelfde eenmalige-per-taxatie toepassing als de woonlagen hierboven.
  bijgebouwenSectieIngeklapt = true;
}
...
function renderWoonlaagKenmerken(woonlaag, wIdx) {
  ...
  // deze is anders dan de lijst die we voor taxatieweb gebouwd hebben") — zelfde ✎-knop +
  // chip-editor als overal elders waar een macro-gekoppelde lijst getoond wordt (bv. Bijgebouwen).
  kaart.appendChild(el('div', { class: 'bouwdeel-veld-label-rij' }, renderMacroBewerkKnop('vloerafwerking')));
  if (bouwdeelChipEditorOpen.vloerafwerking) kaart.appendChild(renderChipEditor(state.macros.vloerafwerking, bewaarMacros));
...
// "⚡ Snel invullen" (19-09-2026, Arno's verzoek) — pop-up per woonlaag, tweede ronde na "op deze
// manier is het compleet": bevat nu ZOWEL de materiaal-kenmerken (Vloersoort/Kozijnen/Glastypes/
// Verwarmingssysteem — dezelfde opslagplek als "Kenmerken verdieping" hierboven, kenmerkenKoppeling)
// ALS de volledige isolatie-/bouwjaar-blokken (Geïsoleerd, Gedeeltelijk geïsoleerd, Installatiemoment,
// Installatiejaar) — 1-op-1 dezelfde velden als Taxatieweb's eigen opnamestaat, zie Arno's voorbeeld:
// "Vloer 1e woonlaag > geïsoleerd Nee/Ja, Gedeeltelijk geïsoleerd Nee/Ja, Bouw-/installatiejaar
// Installatiemoment ...". Schrijft rechtstreeks naar dezelfde t.energetisch-velden en
// woonlaag.kenmerken, dus 100% in sync met Kenmerken verdieping en de Bouwkundig & Energetisch-tab
// (en andersom) — nergens dubbel opgeslagen.
...
// Eigen Ja/Nee-knoppenrij i.p.v. renderJaNeeWissel hergebruiken — die roept zelf altijd de globale
// render() aan (herbouwt #app), terwijl deze pop-up BUITEN #app leeft (net als de foto-lightbox) en
// dus zijn eigen `herteken()` nodig heeft om meteen bij te werken.
function renderPopupJaNeeRij(labelText, huidigeWaarde, onChange) {
  const wissel = el('div', { class: 'weergave-wissel' });
  [[false, 'Nee'], [true, 'Ja']].forEach(([w, tekst]) => {
    wissel.appendChild(el('button', { class: 'klein' + (huidigeWaarde === w ? ' actief' : ''), onclick: () => onChange(w) }, tekst));
  });
  return el('div', { class: 'bouwdeel-conditie-rij' }, el('span', { class: 'energetisch-veld-label' }, labelText), wissel);
}
// Installatiemoment (echte keuzelijst, geen stilzwijgende aanname meer — Arno's correctie: "op deze
// manier is het compleet") + pas bij "Installatiejaar" de jaar-keuze met snelkeuze-chips ERACHTER
// (Arno: "past makkelijk", nowrap+scroll zie .popup-jaar-rij in style.css).
function renderPopupInstallatiemomentEnJaar(veld, herteken) {
  const wrap = el('div', {});
  const select = el('select', {
    class: 'energetisch-select',
    onchange: (e) => { veld.installatiemoment = e.target.value; planOpslaan(); herteken(); },
  },
    el('option', { value: '' }, 'Selecteer'),
    ...INSTALLATIEMOMENT_OPTIES.map((o) => el('option', { value: o, selected: veld.installatiemoment === o ? 'selected' : null }, o)));
  wrap.appendChild(el('label', { class: 'bouwdeel-detail-veld' }, el('span', { class: 'energetisch-veld-label' }, 'Installatiemoment'), select));
  if (veld.installatiemoment === 'Installatiejaar') {
    const kiesJaar = (w) => { veld.jaar = w; planOpslaan(); herteken(); };
    const jaarRij = el('div', { class: 'popup-jaar-rij' }, renderJaarSelect(veld.jaar, kiesJaar));
    alleGebruikteJaartallen().filter((j) => j !== String(veld.jaar || '')).forEach((jaar) => {
      jaarRij.appendChild(el('button', { type: 'button', class: 'chip-knop', onclick: () => kiesJaar(jaar) }, jaar));
    });
    wrap.appendChild(el('label', { class: 'bouwdeel-detail-veld' }, el('span', { class: 'energetisch-veld-label' }, 'Installatiejaar'), jaarRij));
  }
  return wrap;
}
...
// Materiaal-kenmerk + Bouw-/installatiejaar samengevoegd in 1 blokje (20-09-2026, Arno: "glastype
// samenvoegen met jaren, dit past makkelijk") — voor Glas en Verwarmingssysteem, die allebei een
// materiaal-kenmerk (kenmerkenKoppeling) ÉN een eigen bouw-/installatiejaar (materiaalTijd) hebben.
// Geen Gedeeltelijk geïsoleerd hier — dat concept bestaat alleen bij 'isolatie'-type velden (Vloer).
function renderPopupKenmerkenPlusJaarBlok(titel, woonlaag, kenmerkSleutel, jaarVeld, herteken) {
  const blok = el('div', { class: 'snel-invullen-blok' },
    ...
    renderPopupJaNeeRij('Aanwezig', jaarVeld.aanwezig, (w) => { jaarVeld.aanwezig = w; planOpslaan(); herteken(); }),
  );
...
function renderIndelingTab() {
  ...
  t.data.indeling.woonlagen.forEach((woonlaag, wIdx) => {
    ...
    blok.appendChild(el('div', { class: 'woonlaag-kop' },
      ...
      pictogramVoorWoonlaag(woonlaag.naam),
      naamInput,
      // "⚡ Snel invullen" (19-09-2026, Arno's verzoek, tweede ronde: "knop snel invullen in regel
      // woonlaag") — verplaatst van de Kenmerken verdieping-kop naar hier: de pop-up bevat nu ook de
      // materiaal-kenmerken zelf, dus hoort niet meer specifiek bij dat ene sub-blokje.
      el('button', {
...
function renderRuimteKaart(ruimte, verwijder, sleepInfo) {
  ...
  // kleur"; vierde ronde na een scheve/afgeknipte pop-up in de praktijk: "overlay pop-up gaat niet
  // goed") — position:absolute binnen de kaart liep vast op .woonlaag-blok's overflow:hidden (de
  // pop-up viel dan half weg of verkeerd uitgelijnd, zie Arno's screenshot). Nu net als
  // eigenSuggestiesLijst hierboven: position:fixed + getBoundingClientRect van de knop zelf, dus
  // altijd correct t.o.v. het scherm, ongeacht clipping van ouders. Trap alleen tonen als er iets is
  // om "naartoe" te wijzen.
  rijBoven.appendChild(el('button', {
    type: 'button', class: 'ruimte-mini-knop', title: 'Afwerking (vloer/muren/plafond)',
    onclick: (e) => { e.stopPropagation(); openRuimteAfwerkingPopover(ruimte, e.currentTarget); },
  }, '🎨'));
  ...
  if (andereWoonlagen.length) {
    ...
    rijBoven.appendChild(el('button', {
      ...
      onclick: (e) => { e.stopPropagation(); openRuimteTrapPopover(ruimte, andereWoonlagen, standaardDoelNaam, e.currentTarget); },
    }, '🪜'));
  }
  rijBoven.appendChild(el('button', { class: 'verwijder', onclick: verwijder }, '✕'));
  kaart.appendChild(rijBoven);
...
// Afwerking + Trap per ruimte (21-09-2026, Arno n.a.v. de Provadie-rondgang; tweede ronde: "mini
// pop-up direct bij de knop, niet het hele scherm bedekken"; derde ronde: "rechterzijde, compact,
// meer kleur"; vierde ronde na Arno's screenshot van een scheve pop-up: "overlay pop-up gaat niet
// goed") — position:fixed + getBoundingClientRect van de knop (zelfde aanpak als
// eigenSuggestiesLijst hierboven), zodat de pop-up altijd correct t.o.v. het scherm uitgelijnd staat
// en nooit vastloopt op de overflow:hidden van een voorouder-kaart. Eén gedeeld transparant
// klik-vangnet achter het paneel sluit 'm bij een klik erbuiten; een klik op het paneel zelf stopt
// de propagatie zodat dat vangnet niet meteen weer dichtklapt.
function sluitRuimteMiniPopover() {
  const bestaand = document.querySelector('.ruimte-mini-popover-vangnet');
  if (bestaand) bestaand.remove();
}
function positioneerRuimteMiniPaneel(paneel, knopEl) {
  const rect = knopEl.getBoundingClientRect();
  const breedte = 230;
  const links = Math.max(8, Math.min(rect.right - breedte, window.innerWidth - breedte - 8));
  paneel.style.top = (rect.bottom + 6) + 'px';
  paneel.style.left = links + 'px';
  paneel.style.width = breedte + 'px';
}
function openRuimteMiniPopover(knopEl, bouwPaneelInhoud) {
  sluitRuimteMiniPopover();
  const vangnet = el('div', { class: 'ruimte-mini-popover-vangnet', onclick: sluitRuimteMiniPopover });
  const paneel = el('div', { class: 'ruimte-mini-paneel', onclick: (e) => e.stopPropagation() });
  bouwPaneelInhoud(paneel);
  vangnet.appendChild(paneel);
  document.body.appendChild(vangnet);
  positioneerRuimteMiniPaneel(paneel, knopEl);
}
function openRuimteAfwerkingPopover(ruimte, knopEl) {
  openRuimteMiniPopover(knopEl, (paneel) => {
    ...
    paneel.appendChild(maakSelect('Vloer', ruimte.afwerkingVloer, state.macros.vloerafwerking || [], (w) => { ruimte.afwerkingVloer = w; }));
    paneel.appendChild(maakSelect('Muren', ruimte.afwerkingMuren, (state.macros.bouwdeelChips && state.macros.bouwdeelChips.wandenEnBinnenmuren) || [], (w) => { ruimte.afwerkingMuren = w; }));
    paneel.appendChild(maakSelect('Plafond', ruimte.afwerkingPlafond, (state.macros.bouwdeelChips && state.macros.bouwdeelChips.plafonds) || [], (w) => { ruimte.afwerkingPlafond = w; }));
  });
}
...
function openRuimteTrapPopover(ruimte, andereWoonlagen, standaardDoelNaam, knopEl) {
  openRuimteMiniPopover(knopEl, (paneel) => {
    const keuze = { type: TRAP_TYPES[0], doelNaam: standaardDoelNaam || andereWoonlagen[0].naam };
    const typeSelect = el('select', {
      class: 'energetisch-select', onchange: (e) => { keuze.type = e.target.value; },
    }, ...TRAP_TYPES.map((o) => el('option', { value: o, selected: o === keuze.type ? 'selected' : null }, o)));
    const doelSelect = el('select', {
      class: 'energetisch-select', onchange: (e) => { keuze.doelNaam = e.target.value; },
    }, ...andereWoonlagen.map((w) => el('option', { value: w.naam, selected: w.naam === keuze.doelNaam ? 'selected' : null }, w.naam)));
    paneel.appendChild(el('label', { class: 'bouwdeel-detail-veld' }, el('span', { class: 'energetisch-veld-label' }, 'Type'), typeSelect));
    paneel.appendChild(el('label', { class: 'bouwdeel-detail-veld' }, el('span', { class: 'energetisch-veld-label' }, 'Naar'), doelSelect));
    paneel.appendChild(el('button', {
      type: 'button', class: 'knop spook klein', style: 'margin-top:4px;',
      onclick: () => {
        const tekst = `${keuze.type} naar de ${keuze.doelNaam.toLowerCase()}`;
        if (!Array.isArray(ruimte.toevoegingen)) ruimte.toevoegingen = [];
